...
{
    public class OpenHandler : CSharpScript
    {
        private const int MaxErrorTimes = 10;
        private static readonly TimeSpan Interval = TimeSpan.FromMinutes(5);

        public OpenHandler(IScriptContext context)
            : base(context)
        { }
        ...
        void AppInfo()
        {
            ...
            var apps = RowAdapter.Load<App>(p => p.ID == appID && p.Deleted == false);
        ...
        void Authorization()
        {
            ...
            var auths = RowAdapter.Load<Authorization>(p => p.AppID == appID && p.Token == authToken);
            ...
            var accesss = RowAdapter.Load<Access>(p => p.AppID == appID && p.OwnerID == auth.OwnerID);
            var access = accesss.FirstOrDefault();
            if (access == null)
            {
                access = RowAdapter.Create<Access>();
        ...
        void Refresh()
        {
            ...
            var accesss = RowAdapter.Load<Access>(p => p.RefreshToken == token);
            var access = accesss.FirstOrDefault();
            if (access == null)
            {
                WrapResult(false, "invaild access!");
        ...
        void UserInfo()
        {
            ...
            var accesss = RowAdapter.Load<Access>(p => p.AccessToken == token);
            var access = accesss.FirstOrDefault();
            if (access == null)
            {
            ...
            var account = RowAdapter.Load<Account>(p => p.ID == access.OwnerID).FirstOrDefault();
            var json = new JsonObject();
